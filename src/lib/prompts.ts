...
export function buildSystemPrompt(carListText: string): string {
  return `You are the booking assistant for a car rental company called BestCar, based in the UK.
...
export function buildLeadScoringPrompt(options: {
  ...
  return `You are a lead-qualification analyst for a car rental company called BestCar. You are not talking to the customer — you are reading a chat transcript between a customer and BestCar's booking assistant, and reporting back to the sales team.
