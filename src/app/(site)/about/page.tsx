...
export default function AboutPage() {
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-(--space-xl) px-(--space-sm) py-(--space-2xl)">
      <Reveal className="flex flex-col gap-(--space-xs) text-center">
      ...
      <div className="grid grid-cols-1 gap-(--space-sm) sm:grid-cols-3">
        {VALUES.map(({ icon: Icon, title, body }, i) => (
          <Reveal
            key={title}
            delayMs={i * 100}
            className="flex flex-col gap-(--space-2xs) rounded-2xl border border-border p-(--space-sm)"
          >
      ...
      <Reveal className="flex flex-col items-center gap-(--space-xs) rounded-2xl bg-muted p-(--space-lg) text-center">
