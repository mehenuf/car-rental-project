...
| Unit tests | 635 pass (Vitest, 75 files) |
| Browser tests | 101 pass locally; in CI (no database) 76 pass and 23 skip by design |
| CI on `main` | Green at `7a0c1ab` (portal stage 5) and `5435eb6`. `51dfef7` was red because my new unknown-car test ran without a database in CI; fixed by skipping it there (lesson: reproduce CI locally with a placeholder Supabase URL, see section 6). Later commits on 2026-09-23 (vehicle page, Zod removal, cars list) were pushed after `5435eb6`; check the latest run. |
...
| Lighthouse mobile (live) | 2026-09-23 after the region pin and the vehicle-page work: home 89-92, cars 77-92 (bimodal, see the performance log), vehicle 87-88, about 93 (single run). **Target 95 NOT met.** Desktop 96-99 (not re-measured). Started at 54 for home |
...
- Stage 5 is done (2026-09-23): disputes, team, settings, the application form, the onboarding panel and the attestation card, plus document, provider-status and dispute words (143 keys: `portal.disputes/team/settings/docs/apply/attestation`; disputes reuse the customer-side `disputes.status/types/kind`). The front-door notice that said the portal is English only (`providerApply.englishOnly`) was removed. **The whole portal is now translated except:** server-side API error messages (English everywhere; a separate task), the versioned insurance declaration text (`INSURANCE_DECLARATION`, kept in the accepted wording and shown as `lang=en`), the English error from `upload-client.ts` (fleet document upload; the onboarding panel uses the new `documentProblem()` code instead), and the default "please ask a reviewer" body text, which reuses the customer-side line. Nobody has viewed any of it signed in.
- The Terms and Privacy pages are English-only by design (a notice says so in other languages).

**Performance (target mobile 95 not met; expect 85-92 realistically):** remaining weight is React and Next runtime (about 155 KB gz) plus Base UI positioning and menu code needed above the fold. Ideas: render more of the home page without client components; lazy-hydrate the search bar's time selects; replace the three Base UI Selects in the search bar with native selects on touch; verify with `bundle-composition.cjs` and Lighthouse. Cold-start latency and the seeded database round trips also matter. Lighthouse was re-measured on 2026-09-23 (see the fourth pass in the performance log); the vehicle page first load fell from 358 KB to 273 KB gzipped by removing Zod from the booking dialog. Next candidates: the trips page still ships all of Zod (dispute panel), and the cars list score is bimodal because of hydration work on the main thread.
...
3. Ask the owner what to take next; the default is the performance ideas in section 8, then the performance ideas in section 8.
